...
#
# Adapted from the Conan Center recipe for openssl (MIT). Upstream's build is kept as it
...
class OpenSSLConan(ConanFile):
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos", "Android", "iOS"):
            raise ConanInvalidConfiguration(f"openssl is not shipped for {self.settings.os}")
