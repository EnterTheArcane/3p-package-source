...
#
# Adapted from the Conan Center recipe for libsamplerate (MIT). Upstream's build is kept as it
...
class LibsamplerateConan(ConanFile):
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos", "Android", "iOS"):
            raise ConanInvalidConfiguration(
                f"libsamplerate is not shipped for {self.settings.os}")
    ...
    def package_info(self):
        ...
        self.cpp_info.set_property("cmake_file_name", "libsamplerate")
