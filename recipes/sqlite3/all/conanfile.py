...
#
# Adapted from the Conan Center recipe for sqlite3 (MIT). Upstream's build is kept as it
...
class Sqlite3Conan(ConanFile):
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos"):
            raise ConanInvalidConfiguration(f"sqlite3 is not shipped for {self.settings.os}")
    ...
    def package_info(self):
        ...
        self.cpp_info.set_property("cmake_file_name", "SQLite")
