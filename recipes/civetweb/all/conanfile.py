...
#
# Adapted from the Conan Center recipe for civetweb (MIT). Upstream's build is kept as it
...
class CivetwebConan(ConanFile):
    ...
    def validate(self):
        if self.settings.os != "Windows":
            raise ConanInvalidConfiguration(f"civetweb is not shipped for {self.settings.os}")
