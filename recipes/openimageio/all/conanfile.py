...
#
# Adapted from the Conan Center recipe for openimageio (MIT). Upstream's build is kept as it
...
class OpenImageIOConan(ConanFile):
    ...
    # Values differing from upstream's defaults are the engine's choices;
    # see docs/adding-a-package.md on accounting for dependencies.
    default_options = {
        'shared': True,
        'fPIC': True,
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos"):
            raise ConanInvalidConfiguration(
                f"openimageio is not shipped for {self.settings.os}")
