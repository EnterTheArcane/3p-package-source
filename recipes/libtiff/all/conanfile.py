...
#
# Adapted from the Conan Center recipe for libtiff (MIT). Upstream's build is kept as it
...
class LibtiffConan(ConanFile):
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos", "Android", "iOS"):
            raise ConanInvalidConfiguration(f"libtiff is not shipped for {self.settings.os}")
