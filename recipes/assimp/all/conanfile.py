...
#
# Adapted from the Conan Center recipe for assimp (MIT). Upstream's build is kept as it
...
class AssimpConan(ConanFile):
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos"):
            raise ConanInvalidConfiguration(f"assimp is not shipped for {self.settings.os}")
