...
#
# Adapted from the Conan Center recipe for lz4 (MIT). Upstream's build is kept as it
...
class LZ4Conan(ConanFile):
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos", "Android", "iOS"):
            raise ConanInvalidConfiguration(f"lz4 is not shipped for {self.settings.os}")
