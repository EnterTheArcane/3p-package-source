...
#
# Forked from Conan Center's Qt 6 recipe. O3DE-specific changes are intentionally
# limited to the pinned version, module defaults, supported platforms, and retaining
# Qt's native CMake package files for the engine-facing deployment.
...
class QtConan(ConanFile):
    ...
    default_options = {
        ...
        # O3DE's Qt surface. qtbase is implicit and qttranslations pulls qttools.
        "qtimageformats": True,
        "qtsvg": True,
        "qttools": True,
        "qttranslations": True,
    ...
    def config_options(self):
        ...
        if self.settings.os == "Windows":
            ...
            self.options.with_dbus = False
            del self.options.with_gssapi
        elif self.settings.os == "Macos":
            # Match O3DE's existing Qt package: Qt Network uses Secure Transport on
            # macOS, while Linux and Windows link the packaged OpenSSL build.
            self.options.openssl = False
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos"):
            raise ConanInvalidConfiguration(f"qt is not shipped for {self.settings.os}")
    ...
    def requirements(self):
        # Pin dependencies that have O3DE recipes. Version ranges otherwise select a
        # newer Conan Center recipe and bypass the higher-priority local index.
    ...
    def generate(self):
        ...
        # Qt chooses its default deployment target before CMake loads the Conan
        # toolchain. Pass it on the command line so it matches the profile and
        # every prebuilt dependency linked into Qt.
    ...
    def package(self):
        ...
        # Keep the package-level license discoverable by O3DE's archive metadata
        # generator, which intentionally does not recurse through module folders.
        ...
        # Conan Center removes Qt's installed package configs because ordinary Conan
        # consumers use CMakeDeps. O3DE deploys this package outside Conan and its
        # curated qt-config.cmake deliberately consumes those native Qt configs, so
        # retain them in the archive.
