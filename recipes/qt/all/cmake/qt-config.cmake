...
# PAL_PLATFORM_NAME comes from the engine. Derive it when this package is used on its
# own, so the config resolves outside an O3DE build as the other packages do.
if (NOT PAL_PLATFORM_NAME)
    if (APPLE)
...
foreach(component ${QT6_COMPONENTS})
    if(TARGET Qt6::${component})
        ...
        # O3DE patches SYSTEM includes for toolchains that get them wrong; fall back to
        # plain CMake so this package also resolves outside the engine.
        if (COMMAND ly_target_include_system_directories)
            ly_target_include_system_directories(TARGET Qt6::${component} INTERFACE ${system_includes})
        else()
            target_include_directories(Qt6::${component} SYSTEM INTERFACE ${system_includes})
        endif()
...
# Deployment is the engine's job; guarded so this package also resolves standalone.
if(tranlation_files AND COMMAND ly_add_target_files)
    ly_add_target_files(TARGETS 3rdParty::Qt::Core::Translations
        FILES ${tranlation_files}
        OUTPUT_SUBDIRECTORY translations
    )
endif()
if(COMMAND ly_add_dependencies)
...
# Report the package as found, the way every other package here does. Qt exports its
# components as 3rdParty::Qt::<Component> and never a bare 3rdParty::Qt target, so this
# variable is what tells a consumer the module resolved.
set(Qt_FOUND True)
