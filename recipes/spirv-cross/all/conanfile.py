...
#
# Adapted from the Conan Center recipe for spirv-cross (MIT). Upstream's build is kept as it
...
class SpirvCrossConan(ConanFile):
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos"):
            raise ConanInvalidConfiguration(
                f"spirv-cross is not shipped for {self.settings.os}")
    ...
    def package_info(self):
        ...
        self.cpp_info.set_property("cmake_file_name", "SPIRVCross")
