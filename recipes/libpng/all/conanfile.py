...
#
# Adapted from the Conan Center recipe for libpng (MIT). Upstream's build is kept as it
...
class LibpngConan(ConanFile):
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos", "Android", "iOS"):
            raise ConanInvalidConfiguration(f"libpng is not shipped for {self.settings.os}")
