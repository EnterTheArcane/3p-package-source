...
#
# Adapted from the Conan Center recipe for openexr (MIT). Upstream's build is kept as it
...
class OpenEXRConan(ConanFile):
    name = "openexr"
    version = "3.4.13"
    # openexr's own requirements, minus zlib which ships as its own package. They have
    # no option to opt out of, so they travel inside this one. Only Imath stays visible:
    # the engine has always registered both names.
    ...
    def validate(self):
        if str(self.settings.os) not in ("Windows", "Linux", "Macos"):
            raise ConanInvalidConfiguration(f"openexr is not shipped for {self.settings.os}")
